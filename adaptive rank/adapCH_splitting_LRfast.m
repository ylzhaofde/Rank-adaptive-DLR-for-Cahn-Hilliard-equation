% adaptive low-rank splitting method for CH equation
...
% adaptive low-rank approx for initial value
adap_rank = zeros(M+1,1);
[U,S,V,adap_rank(1)] = truncated_svd(u(1:Nx,1:Ny,1),[1e-11,1e-12]);
u(1:Nx,1:Ny,1) = U*S*V';
for j = 1:M
    % nonlinear part:
    v = tau*(fu(u(1:Nx,1:Ny,j))*Ay + Ax*fu(u(1:Nx,1:Ny,j))) + u(1:Nx,1:Ny,j);
    [U_v,S_v,V_v,adap_rank(j + 1)] = adapDLRA_proj_splitting(U,S,V,v);
    % linear part:
    v = expA2vec(U_v*S_v*V_v');
    [U,S,V] = DLRA_proj_splitting(U_v,S_v,V_v,v);
...
fprintf('Adaptive Low-rank splitting for CH, Example 1\n')
fprintf('CPU: %.3f\n',interval)
